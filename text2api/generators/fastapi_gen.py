...
""",
    ...
    async def generate(self, api_spec: ApiSpec, output_path: Path) -> Dict[str, str]:
        ...
        req_content = self.templates.get_template("requirements.txt").render(
            api_spec=api_spec
        )
        ...
        if api_spec.database_required:
            models_content = self.templates.get_template("models.py").render(
                api_spec=api_spec
            )
        ...
        # Generuj .env.example
        env_content = self.templates.get_template(".env.example").render(
            api_spec=api_spec
        )
    ...
    async def _generate_utils(
        self, api_spec: ApiSpec, output_path: Path, generated_files: Dict[str, str]
    ):
