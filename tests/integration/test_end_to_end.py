...
class TestEndToEndGeneration:
    ...
    @pytest.mark.asyncio
    async def test_generate_with_complex_requirements(self, temp_dir, mock_ollama_client):
    ...
    async def test_language_detection_integration(self, temp_dir, mock_ollama_client):
        ...
        for description, expected_lang in test_cases:
            with patch.object(generator.text_analyzer.language_detector, 'detect_language', return_value=expected_lang):
                result = await generator.generate_from_text(
                    text=description,
                    output_name=f"api_{expected_lang}"
                )
...
class TestFileOperationsIntegration:
    ...
    async def test_directory_synchronization(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result["success"] is True if "success" in result else len(result["errors"]) == 0
...
class TestGeneratorIntegration:
    ...
    async def test_docker_file_validity(self, sample_api_spec, temp_dir):
        ...
        instructions = [line.split()[0].upper() for line in lines if line.strip() and not line.startswith('#')]
