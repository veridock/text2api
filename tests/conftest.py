...
class TestTextAnalyzer:
    ...
    async def test_analyze_text_with_language_detection(self, mock_ollama_client):
        ...
        for text, expected_lang in test_cases:
            with patch.object(analyzer.language_detector, 'detect_language', return_value=expected_lang):
                result = await analyzer.analyze_text(text)
    ...
    async def test_analyze_text_ollama_error(self, mock_ollama_client):
        ...
        # Mock Ollama error
        mock_ollama_client.analyze_api_requirements.side_effect = Exception("Ollama error")
...
class TestOllamaClient:
    ...
    async def test_analyze_api_requirements(self):
        ...
        with patch.object(client, 'ensure_model', return_value=True), \
             patch.object(client, 'generate', return_value='{"api_type": "rest"}'):
    ...
    async def test_analyze_api_requirements_invalid_json(self):
        ...
        with patch.object(client, 'ensure_model', return_value=True), \
             patch.object(client, 'generate', return_value='Invalid JSON'), \
...
class TestLanguageDetector:
    ...
    def test_get_language_info(self):
        ...
        with patch.object(detector, 'detect_language', return_value='pl'), \
             patch.object(detector, 'get_confidence', return_value=0.8):
...
class TestValidation:
    ...
    def test_validate_project_name(self):
        ...
        invalid_names = ["", "a", "very-long-name-" + "x" * 50, "123project", "project name"]
    ...
    def test_suggest_alternative_port(self):
        ...
        with patch('text2api.utils.validation.check_port_availability', side_effect=mock_check_port):
