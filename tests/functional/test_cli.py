...
class TestCLIFunctional:
    ...
    def test_cli_generate_command(self, mock_generate):
        ...
        result = runner.invoke(
            cli,
            ["generate", "API do zarządzania użytkownikami", "--output", "test_api"],
        )

        assert result.exit_code == 0
        assert (
            "wygenerowane pomyślnie" in result.output
            or "generated successfully" in result.output
        )
    ...
    def test_cli_interactive_mode_input(self):
        ...
        with patch(
            "text2api.core.generator.APIGenerator.generate_from_text"
        ) as mock_generate:
            mock_generate.return_value = {"success": True, "project_path": "./test"}

            result = runner.invoke(
                cli, ["generate-from-file", "--interactive"], input="\n".join(inputs)
            )
...
class TestPerformance:
    ...
    async def test_code_generation_performance(self, sample_api_spec, temp_dir):
        ...
        assert (
            generation_time < 1.0
        ), f"Generation took {generation_time:.2f}s, expected < 1.0s"
        assert len(result) > 0
    ...
    async def test_large_api_spec_generation(self, temp_dir):
        ...
        assert (
            generation_time < 5.0
        ), f"Large API generation took {generation_time:.2f}s"
        assert len(result) > 0
        ...
        endpoint_count = sum(
            1 for endpoint in endpoints if endpoint.path in main_content
        )
        assert (
            endpoint_count >= 40
        ), f"Only {endpoint_count}/50 endpoints found in generated code"
    ...
    async def test_file_io_performance(self, temp_dir):
        ...
        assert (
            write_time < 2.0
        ), f"Writing 100 files took {write_time:.2f}s, expected < 2.0s"
        ...
        assert (
            read_time < 1.0
        ), f"Reading 100 files took {read_time:.2f}s, expected < 1.0s"
...
class TestRealWorldScenarios:
    ...
    async def test_ecommerce_api_scenario(self, temp_dir, mock_ollama_client):
        ...
        result = await generator.generate_from_text(
            text=description, output_name="ecommerce_api"
        )
    ...
    async def test_microservice_scenario(self, temp_dir, mock_ollama_client):
        ...
        result = await generator.generate_from_text(
            text=description, output_name="user_service"
        )
...
class TestErrorScenarios:
    ...
    async def test_invalid_text_input(self, temp_dir, mock_ollama_client):
        ...
        for invalid_input in invalid_inputs:
            result = await generator.generate_from_text(
                text=invalid_input, output_name="invalid_test"
            )
    ...
    async def test_ollama_timeout_error(self, temp_dir):
        ...
        # Mock timeout error
        with patch.object(
            generator.ollama_client, "health_check", side_effect=asyncio.TimeoutError()
        ):
            result = await generator.generate_from_text(
                text="Simple API", output_name="timeout_test"
            )
    ...
    async def test_file_permission_error(self, temp_dir, mock_ollama_client):
        ...
        with patch.object(
            generator.file_manager,
            "write_file",
            side_effect=PermissionError("Permission denied"),
        ):
            result = await generator.generate_from_text(
                text="Simple API", output_name="permission_test"
            )
    ...
    async def test_invalid_output_path(self, mock_ollama_client):
        ...
        result = await generator.generate_from_text(
            text="Simple API", output_name="invalid_path_test"
        )
    ...
    async def test_malformed_ollama_response(self, temp_dir):
        ...
        with patch.object(
            ...
            result = await generator.generate_from_text(
                text="Simple API", output_name="malformed_test"
            )
    ...
    async def test_network_error_handling(self, temp_dir):
        ...
        with patch.object(
            ...
            result = await generator.generate_from_text(
                text="Simple API", output_name="network_test"
            )
...
class TestEdgeCases:
    ...
    async def test_very_long_description(self, temp_dir, mock_ollama_client):
        ...
        long_description = (
            "API for managing " + "very complex business processes " * 200
        )

        result = await generator.generate_from_text(
            text=long_description, output_name="long_description_test"
        )
    ...
    async def test_special_characters_in_description(
        ...
        for i, description in enumerate(special_descriptions):
            result = await generator.generate_from_text(
                text=description, output_name=f"special_chars_test_{i}"
            )
    ...
    async def test_mixed_language_description(self, temp_dir, mock_ollama_client):
        ...
        for i, description in enumerate(mixed_descriptions):
            result = await generator.generate_from_text(
                text=description, output_name=f"mixed_lang_test_{i}"
            )
    ...
    async def test_duplicate_project_names(self, temp_dir, mock_ollama_client):
        ...
        result1 = await generator.generate_from_text(
            text="User management API", output_name="duplicate_test"
        )
        ...
        result2 = await generator.generate_from_text(
            text="Product management API", output_name="duplicate_test"
        )
        ...
        assert (
            Path(result1["project_path"]).parent == Path(result2["project_path"]).parent
        )
    ...
    async def test_maximum_entities_and_endpoints(self, temp_dir, mock_ollama_client):
        ...
        result = await generator.generate_from_text(
            text=description, output_name="max_complexity_test"
        )
